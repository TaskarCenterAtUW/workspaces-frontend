<template>
  <section class="review-sidebar">
    <header class="border-top">
      <h2 class="h5">Reviewing Workspace</h2>
      <review-filter-dropdown v-model="filter" @apply="refresh" />
    </header>

    <header class="border-top border-bottom">
      <h3>{{ props.reviewList.workspace?.title }}</h3>
      <button class="btn btn-light btn-sm border ms-auto" @click="refresh">
        <app-icon variant="refresh" no-margin />
        <span class="visually-hidden">Refresh</span>
      </button>
    </header>

    <div ref="reviewListGroup" class="list-group">
